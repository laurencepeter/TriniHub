...
--  for the tables and 20240101001100_admin_policies.sql for trinihub.is_admin().)

begin;

-- Enable RLS on dog registration tables.
alter table trinihub.regions enable row level security;
alter table trinihub.breeds enable row level security;
alter table trinihub.owners enable row level security;
alter table trinihub.dogs enable row level security;
alter table trinihub.dog_ownerships enable row level security;

-- Regions/Breeds: allow authenticated users to read lookup data.
drop policy if exists regions_select_all on trinihub.regions;
...
drop policy if exists breeds_select_all on trinihub.breeds;
...
drop policy if exists owners_insert_own on trinihub.owners;
...
drop policy if exists owners_select_own on trinihub.owners;
...
drop policy if exists dogs_insert_owner on trinihub.dogs;
...
drop policy if exists dogs_select_owner on trinihub.dogs;
...
drop policy if exists dog_ownerships_insert_owner on trinihub.dog_ownerships;
create policy dog_ownerships_insert_owner
on trinihub.dog_ownerships
for insert
to authenticated
with check (
  exists (
...
drop policy if exists dog_ownerships_select_owner on trinihub.dog_ownerships;
...
drop policy if exists owners_admin_all on trinihub.owners;
...
drop policy if exists dogs_admin_all on trinihub.dogs;
...
drop policy if exists dog_ownerships_admin_all on trinihub.dog_ownerships;
