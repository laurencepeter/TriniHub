...
-- 1. Custody columns -------------------------------------------------------
alter table trinihub.file_scans
...
update trinihub.file_scans set actor_id = user_id where actor_id is null;
update trinihub.file_scans set custodian_id = user_id where custodian_id is null;

-- 3. Department is optional for personal hand-offs -------------------------
alter table trinihub.file_scans alter column department_id drop not null;

-- 4. user_id is now derived from actor_id; keep it but allow null ----------
alter table trinihub.file_scans alter column user_id drop not null;

-- 5. Expand the allowed event types (legacy in/out preserved) --------------
alter table trinihub.file_scans drop constraint if exists file_scans_event_type_check;
alter table trinihub.file_scans
...
drop policy if exists "Authenticated insert custody" on trinihub.file_scans;
create policy "Authenticated insert custody" on trinihub.file_scans
  for insert
...
drop policy if exists "Authenticated update files" on trinihub.files;
create policy "Authenticated update files" on trinihub.files
  for update using (auth.role() = 'authenticated') with check (auth.role() = 'authenticated');

-- 8. Fast per-file custody history ----------------------------------------
create index if not exists file_scans_file_time_idx
  on trinihub.file_scans (file_id, scan_time desc);
