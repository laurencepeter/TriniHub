...
create unique index if not exists issues_issue_fingerprint_idx on public.issues (issue_fingerprint)
...
create index if not exists issue_votes_issue_id_idx on public.issue_votes (issue_id);

-- Ensure unique votes per user/device
create unique index if not exists issue_votes_unique_voter_id on public.issue_votes (issue_id, voter_id)
  where voter_id is not null;
create unique index if not exists issue_votes_unique_voter_hash on public.issue_votes (issue_id, voter_hash)
  where voter_hash is not null;

-- Popularity view for quick aggregation
create or replace view public.issue_popularity as
select
...
from public.issues
left join public.issue_reports on issue_reports.issue_id = issues.id
left join public.issue_votes on issue_votes.issue_id = issues.id
group by issues.id;
