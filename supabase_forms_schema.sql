...
drop policy if exists "Admin update templates" on public.form_templates;
drop policy if exists "Admin delete templates" on public.form_templates;
...
drop policy if exists "Admin insert fields" on public.form_fields;
drop policy if exists "Admin update fields" on public.form_fields;
drop policy if exists "Admin delete fields" on public.form_fields;
...
create policy "Admin update templates" on public.form_templates
  for update using (public.current_app_role() = 'admin')
  with check (public.current_app_role() = 'admin');

create policy "Admin delete templates" on public.form_templates
...
create policy "Admin insert fields" on public.form_fields
  for insert
  with check (public.current_app_role() = 'admin');

create policy "Admin update fields" on public.form_fields
  for update using (public.current_app_role() = 'admin')
  with check (public.current_app_role() = 'admin');

create policy "Admin delete fields" on public.form_fields
  for delete using (public.current_app_role() = 'admin');
    ...
    exists (
      ...
      where s.id = submission_id
        and (
          ...
          or s.submitter_id = (select auth.uid())
        )
    )
  );
